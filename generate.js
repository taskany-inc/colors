...
const writeToShadowDir = (file, content) =>
  mkdirpAsync(dirname(file), { recursive: true }).then(() =>
    writeFileAsync(file, content, "utf-8")
  );
const themesFolder = join(process.cwd(), "generated");
const themes = require("./palette");

const jsToken = (token, value, { noVar } = {}) =>
  `export const ${camelCase(token)} = '${
    noVar ? value : `var(--${paramCase(token)})`
  }';`;
const cssToken = (token, value, tabs = 8) =>
  `${" ".repeat(tabs)}--${paramCase(token)}: ${value};\n`;
...
const indexFile = (themes, file) => {
  ...
  themes.forEach(theme => {
    fileContent.push(`export * as ${capitalizeFirstLetter(theme)}Theme from './${theme}'\n`);
  });
    ...
    for (const token in t[theme]) {
      themes.forEach((themeName) => {
        if (!t[themeName][token]) {
          console.log("\n");
          console.warn(
            `😞 Token: "${token}" doesn't exist in theme "${themeName}"!`
          );
        }
      });
    }
